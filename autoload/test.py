...
def test_proof():
    ...
    assert get_goals() == Goals(
        [Goal('3', ['n : nat'], 'n + 0 = n')], [], [], [])
    advance(cmd4)
    assert get_goals() == Goals([Goal('7', [], '0 + 0 = 0'), Goal(
        '10', ["n' : nat", "IHn' : n' + 0 = n'"], "S n' + 0 = S n'")], [], [], [])
    advance(cmd5)
    assert get_goals() == Goals(
        [Goal('10', ["n' : nat", "IHn' : n' + 0 = n'"], "S n' + 0 = S n'")], [], [], [])
    advance(cmd6)
    assert get_goals() == Goals(
        [Goal('13', ["n' : nat", "IHn' : n' + 0 = n'"], "S (n' + 0) = S n'")], [], [], [])
    advance(cmd7)
    assert get_goals() == Goals(
        [Goal('14', ["n' : nat", "IHn' : n' + 0 = n'"], "S n' = S n'")], [], [], [])
    advance(cmd8)
    assert get_goals() == Goals([], [], [], [])
    advance(cmd9)
